...
from cp_reach.lie.so3 import Euler
...
def find_Q(deriv_order, poly_deg, n_legs):
    ...
    beta = sympy.symbols("beta")           # normalized time (0 to 1)
    T = sympy.symbols("T")                 # duration of one segment
    c = sympy.MatrixSymbol("c", n_c, 1)    # vector of polynomial coefficients

    # Define the m-th derivative of the polynomial at time beta
    expr = sympy.summation(
        c[k, 0] * sympy.factorial(k) / sympy.factorial(k - m) * beta**(k - m) / T**m,
        (k, m, n)
    )

    # Substitute derivative order and polynomial degree
    expr = expr.subs({m: deriv_order, n: poly_deg}).doit()

    # Integrate squared expression over normalized time
    J_expr = sympy.integrate(expr**2, (beta, 0, 1)).doit()

    # Build cost vector p = [c0, c1, ..., cN]
    p = sympy.Matrix([c[i, 0] for i in range(poly_deg + 1)])

    # Compute symbolic Hessian: Q = 0.5 * ∇²J = 0.5 * ∇(∇J)^T
    Q = sympy.Matrix([J_expr]).jacobian(p).jacobian(p) / 2

    # Assert that this Q recovers the cost when used as p.T @ Q @ p
    assert (p.T @ Q @ p)[0, 0].expand() == J_expr

    # Create symbolic vector of time variables: T = [T_0, ..., T_{n_legs-1}]
    Ti = sympy.MatrixSymbol("T", n_legs, 1)

    # Replace T with T[i] for each segment and build block-diagonal Q
    Q_blocks = [Q.subs(T, Ti[i]) for i in range(n_legs)]
    Q_block_diag = sympy.diag(*Q_blocks)
...
def plan_trajectory(bc, cost, n_legs, poly_deg, k_time, T_opt=None):
    ...
    for j in range(x.shape[0]):
        ...
        R = ref_v[1]
        theta = np.array(ca.DM(Euler.from_dcm(R))).reshape(3,)
