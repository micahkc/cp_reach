...
def solve(vel_dist, accel_dist, ang_accel_dist, ref, dynamics_sol=None, kinematics_sol=None):
    """
    ...
    Parameters:
        vel_dist (float):
            Upper bound on velocity disturbance (m/s), e.g., from optical sensor attack.
    ...
    Returns:
        ...
        kinematics_sol    : dict
            Solution to the Lyapunov LMI for SE(3) kinematics (contains P, mu1, mu2, mu3, etc.).
    """
    if ref is None:
        ax_max = [0]
        ay_max = [0]
    ...
    val_kin = mu1*vel_dist**2 + mu2 * accel_dist**2 + mu3 * omega_dist**2
