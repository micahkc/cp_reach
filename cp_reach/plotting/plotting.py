...
def plot_error_bounds_2d(points_algebra, points_group):
    """
    Plot 2D projections of invariant error bounds in both Lie algebra and Lie group coordinates.

    Parameters
    ----------
    points_algebra : ndarray (6, N)
        Points in Lie algebra coordinates (error representation)
    points_group : ndarray (6, N)
        Points in Lie group coordinates (state space representation)
    ref_traj : dict, optional
        Reference trajectory with keys 'x', 'y'. If provided, overlays reference
        on the Lie group plot.

    Returns
    -------
    fig : matplotlib.figure.Figure
        The created figure containing both subplots
    """
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))

    # Lie Algebra (error coordinates)
    ax1.plot(points_algebra[0], points_algebra[1], 'g-', linewidth=2, label='Invariant Set')
    ax1.set_xlabel(r'$\zeta_x$ (m)', fontsize=12)
    ax1.set_ylabel(r'$\zeta_y$ (m)', fontsize=12)
    ax1.set_title('Invariant Set in Lie Algebra', fontsize=14, fontweight='bold')
    ax1.axis('equal')
    ax1.grid(True, alpha=0.3)
    ax1.legend()

    # Lie Group (state space)
    ax2.plot(points_group[0], points_group[1], 'g-', linewidth=2, label='Invariant Set')


    ax2.set_xlabel(r'$\eta_x$ (m)', fontsize=12)
    ax2.set_ylabel(r'$\eta_y$ (m)', fontsize=12)
    ax2.set_title('Invariant Set in Lie Group', fontsize=14, fontweight='bold')
    ax2.axis('equal')
